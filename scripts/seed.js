...
async function seed() {
  ...
  await db.execute("DELETE FROM service_plans");
  ...
  for (const p of products) {
    await db.execute({
      sql: `INSERT INTO products (name, slug, description, type, price, original_price, download_url, tags, is_free)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [p.name, p.slug, p.description, p.type, p.price, p.originalPrice, p.downloadUrl, p.tags, p.isFree],
    });
  }
  ...
  for (const p of plans) {
    await db.execute({
      sql: `INSERT INTO service_plans (name, price, period, features, is_popular, stripe_price_id, sort_order)
            VALUES (?, ?, ?, ?, ?, ?, ?)`,
      args: [p.name, p.price, p.period, p.features, p.isPopular, p.stripePriceId, p.sortOrder],
    });
  }
  ...
  for (const f of faqs) {
    await db.execute({
      sql: `INSERT INTO faq_items (question, answer, category, sort_order)
            VALUES (?, ?, ?, ?)`,
      args: [f.question, f.answer, f.category, f.sortOrder],
    });
  }
  ...
  for (const b of blogPosts) {
    await db.execute({
      sql: `INSERT INTO blog_posts (slug, title, description, content, tags, published_at)
            VALUES (?, ?, ?, ?, ?, ?)`,
      args: [b.slug, b.title, b.description, b.content, b.tags, b.publishedAt],
    });
  }
