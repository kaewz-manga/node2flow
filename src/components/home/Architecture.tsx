...
function IconCard({ src, label }: { src: string; label: string }) {
  return (
    <div className="group card-glow flex flex-col items-center gap-1.5 p-3.5 px-2 bg-n2f-secondary border border-n2f-border rounded-xl hover:border-n2f-border-hover hover:bg-n2f-hover transition-all duration-300">
      <Image src={src} alt={label} width={36} height={36} className="h-9 w-auto opacity-70 group-hover:opacity-100 transition-opacity duration-300" />
...
function Connector() {
  return (
    <div className="flex-shrink-0 basis-24 h-10 flex items-center max-[968px]:rotate-90 max-[968px]:h-[60px] max-[968px]:basis-auto max-[968px]:w-10">
      <svg viewBox="0 0 100 40" preserveAspectRatio="none" className="w-full h-full overflow-visible">
        <defs>
          <linearGradient id="arrowGradArch" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="#ff6d5a" stopOpacity={0.3} />
            <stop offset="100%" stopColor="#ff6d5a" stopOpacity={1} />
          </linearGradient>
        </defs>
        <line
          x1={0} y1={20} x2={82} y2={20}
          stroke="url(#arrowGradArch)" strokeWidth={2.5}
          strokeDasharray="6,4"
          className="animate-[arrowFlow_0.5s_linear_infinite]"
        />
        <polygon points="82,14 98,20 82,26" fill="#ff6d5a" opacity={1} />
      </svg>
    </div>
  );
}
          ...
          <div className="flex items-center justify-center gap-0 max-[968px]:flex-col max-[968px]:gap-3">
            ...
            <div className="flex-1 max-w-[300px] max-[968px]:max-w-[400px] max-[968px]:w-full">
              <h3 className="text-xs font-medium text-n2f-accent text-center uppercase tracking-[2px] mb-4">
                <span className="border-b border-n2f-accent/30 pb-1">{t("aiClients")}</span>
              </h3>
              ...
              <div className="gradient-border p-8 px-7 text-center shadow-[0_0_60px_rgba(255,109,90,0.08)]">
                <div className="relative inline-block mb-3">
                  <Image
                    ...
                    width={80}
                    height={80}
                    className="rounded-full border-2 border-n2f-accent/40 shadow-[0_0_30px_rgba(255,109,90,0.15)]"
                  />
                  <Image
                ...
                <h3 className="text-xl font-bold text-n2f-text mb-1">{t("hubName")}</h3>
                <p className="text-sm text-n2f-accent font-semibold">{t("hubLabel")}</p>
              </div>
            </div>
